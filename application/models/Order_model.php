...
class Order_model extends CI_Model{
    ...
    public function edit_order($table_name,$order_id) {
    try {
    if (!empty($table_name) && !empty($order_id)) {

    $query = $this->db->get_where($table_name, array('order_id' => $order_id))->row();
    return $query;
...
public function update_order_item($item,$item_id) {
    try {
    if ($item_id!='') { 
    $this->db->where('item_id',$item_id);
    return $this->db->update('order_items',$item);
